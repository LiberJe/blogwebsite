...
function update(req,res) {
    ...
    req.on("end",function(data){
        var val=JSON.parse(chunks);
        console.log(val.title);
        var condition="UPDATE articlelist SET content=?,title=?,date=? WHERE id=?";
        var conditonval=[val.content,val.title,val.date,val.id];
        connection.query(condition,conditonval,function(err,data){
            if(err){
                console.error("err");
                var dpdata={"check":false};
                res.writeHead(200,{"Content-Type":"application/json"});
